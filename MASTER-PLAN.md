...
## Database access
The real `DATABASE_URL` is injected at runtime by the Neon Vercel integration and is **not** exposed by `vercel env pull` — that command returns it blank.

To run scripts locally against the real database:
1. Vercel → wintergarten → Settings → Environment Variables → copy `DATABASE_URL`
2. Paste into `.env.local` as `DATABASE_URL=<value>` (already in `.gitignore`)
3. Run scripts normally — they read from `.env.local`

The string `postgres://default:g9ykUX8mNbli@ep-round-river-a5pzyn9u…` in prior session notes is a **different database** (stale/dev). Do not use it. It has different data and will not affect what derwintergarten.com serves.
...
| SITE-AUDIT.md | Point-in-time bug list from 2026-08-20 audit — superseded by bug queue above | ❌ DELETE after confirming bugs migrated here |
| ADMIN-WIRING-PLAN.md | Superseded — all content consolidated here | ❌ DELETE |
...
| HANDOVER.md | Owner-facing: day-to-day admin tasks (note: some info is now outdated — ingredients now editable in admin) | ✅ KEEP but update |
